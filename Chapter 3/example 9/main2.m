...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % worst
over_approx = sim_E(T_sim, X0(:), U, @d_norm);
...
patch(corn(1, k), corn(2, k), 'b', 'FaceAlpha', .1);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
over_approx = sim_E(T_sim, X0(:), U, @d);
over_approx = reshape(over_approx, [3, 2]);
...
k = convhull(corn(1, :)', corn(2, :)');
patch(corn(1, k), corn(2, k), 'w');
patch(corn(1, k), corn(2, k), [1, 1, 0], 'FaceAlpha', .1);


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
over_approx = sim_E(T_sim, X0(:), U, @d_best);
over_approx = reshape(over_approx, [3, 2]);
...
matlab2tikz('unicycle_2.tikz', 'width', '6cm', 'height', '4cm')
