...
k = boundary(holder(1, :)', holder(2, :)',0.02);
x_T = holder(:, k);
xu_T = xu(:, t + 1);
xo_T = xo(:, t + 1);

k = boundary(nholder(1, :)', nholder(2, :)',0.02);
nx_T = nholder(:, k);
nxu_T = nxu(:, t + 1);
nxo_T = nxo(:, t + 1);
...
SF = [xu_T(1), xo_T(1), xo_T(1), xu_T(1), xu_T(1); ...
      xu_T(2), xu_T(2), xo_T(2), xo_T(2), xu_T(2)];
patch(SF(1, :), SF(2, :), 'g', 'FaceAlpha', .1, 'LineWidth', 1.25);

holder = x_T;
x_T = holder(:, 1);
for i = 2:size(holder, 2)
    if norm(holder(:, i - 1) - holder(:, i)) >= .001
        x_T(:, end+1) =  holder(:, i);
    else
    end
end
patch(x_T(1, 1:end), x_T(2, 1:end), 'g', 'FaceAlpha', .7, 'LineWidth', 1.25);

scatter([xu_T(1, 1), xo_T(1, 1)], [xu_T(2, 1), xo_T(2, 1)], 'k', 'filled');

% Plot backward time reachable set and MM approximation

NSF = [nxu_T(1), nxo_T(1), nxo_T(1), nxu_T(1), nxu_T(1); ...
       nxu_T(2), nxu_T(2), nxo_T(2), nxo_T(2), nxu_T(2)];
patch(NSF(1, :), NSF(2, :), 'b', 'FaceAlpha', .05, 'LineWidth', 1.25);
holder = nx_T;
nx_T = holder(:, 1);
for i = 2:size(holder, 2)
    if norm(holder(:, i - 1) - holder(:, i)) >= .0006
        nx_T(:, end+1) =  holder(:, i);
    else
    end
end
patch(nx_T(1, 1:end), nx_T(2, 1:end), 'b', 'FaceAlpha', .6, 'LineWidth', 1.25);
...
matlab2tikz('backward.tikz', 'width', '6cm', 'height', '4cm')
