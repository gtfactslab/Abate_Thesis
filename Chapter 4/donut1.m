...
holder = outer_REACH(:, 1);
for i = 2:size(outer_REACH, 2)
    if norm(holder(:, end) - outer_REACH(:, i)) >= .05
        holder(:, end + 1) = outer_REACH(:, i);
    end
end
patch(holder(1, :), holder(2, :), 'w', ...
                                            'HandleVisibility', 'off', ...
                                            'FaceAlpha', 1);
patch(holder(1, :), holder(2, :), 'b',  ...
                                            'HandleVisibility', 'off', ...
                                            'FaceAlpha', .5); % outer ring

holder = inner_REACH(:, 1);
for i = 2:size(inner_REACH, 2)
    if norm(holder(:, end) - inner_REACH(:, i)) >= .05
        holder(:, end + 1) = inner_REACH(:, i);
    end
end
patch(holder(1, :), holder(2, :), 'w', ...
                                            'HandleVisibility', 'off', ...
...
matlab2tikz('donut1.tikz', 'width', '6cm', 'height', '4cm')
