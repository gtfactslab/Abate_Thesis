...
[X,Y] = meshgrid(-1.5:.25:1.5,-1.5:.25:1.5);

for i = 1:size(X, 1)
    for j = 1:size(X, 2)
        thing = F([X(i, j), Y(i, j)], [0; 0]);
        U(i, j) = thing(1);
        V(i, j) = thing(2);
...
quiver(X,Y,Un,Vn, 0.24, 'k', 'LineWidth', 1.6)
set(gcf,'color','w');

%, matlab2tikz('invariant_set.tikz', 'width', '6cm', 'height', '4cm')
                                    
   
x0 = [1; 1.5];
X = x0;
for i = 1:1000
    X(:, i+1) = X(:, i) + .01*F(X(:, i), [0; 0]);
end

plot(X(1, :), X(2, :))

matlab2tikz('attractive_set.tikz', 'width', '6cm', 'height', '4cm')
